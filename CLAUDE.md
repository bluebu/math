...
这不是普通代码仓库，而是一份**小学三年级数学（人教版 RJ，2024 新版，下册）学习资料 + 讲义站点**。核心产物是一套给三年级孩子的「计算秘籍」讲义（HTML，A4 打印友好），由原始真题整理而来，并通过 GitHub Pages 发布。没有构建 / 测试 / lint 流程，不要凭空创造这类命令。
...
## 三年级下册单元与题型（人教版 2024 新版）

⚠️ 用 **2024 新版**的单元结构判断题目归属，**不要再套旧版目录**（旧版的「位置与方向」「两位数乘两位数」「复式统计表」「数学广角—搭配」等已被整合或移走）。新版按目录顺序为：

- 一、**生活中的运动现象**（p1）——平移、旋转、轴对称等图形的运动
- 二、**除数是一位数的除法**（p10）——口算/笔算除法、商中间或末尾有 0、有余数除法、估算、归一与归总问题
- 三、**长方形和正方形**（p39）——图形特征、周长
- 四、**图形的面积**（p52）——面积单位、长方形/正方形面积、面积与周长的辨析
- 五、**数据的收集与整理**（p65）——统计表、条形统计图
- （综合实践专题）**年、月、日的秘密**（p77）——年月日、平年闰年、24 时计时法、经过时间与日期推算（如三伏天天数）
- 六、**小数的初步认识**（p86）——小数含义（如 0.4）、读写、比较、简单加减
- 七、**复习与关联**（p95）——综合复习

讲解时按对应单元选用标准解法：用算术方法而非代数方程、避免超纲。
